...
private:
    ...
    // ADD MORE DATA MEMBERS HERE, AS NECESSARY
		...
		unsigned int toDelete_;
		unsigned int toNotDelete_;
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    // Initialize any other data members as necessary
		...
		toDelete_ = 0;
		toNotDelete_ = 0;
		std::vector<HashItem*> temp(CAPACITIES[0], NULL);
		table_ = temp;
}
...
{
	double items = toDelete_ + toNotDelete_; //total items in table

	if(items / table_.size() >= loadingfactor){
		resize();
	}
	...
	if(h == npos){ //can't add
		throw std::logic_error("available location not found");
	}else if(table_[h] != NULL){ //table has item
		table_[h]->item.second = p.second;
	}else{ //table doesn't have item so add it
...
{
	std::vector<HashItem*> largerTemp(CAPACITIES[mIndex_+1], NULL);//new larger temp all values NULL
	std::vector<HashItem*> oldCopy(table_);//copy table into old copy
	
	table_ = largerTemp; //set table to larger new empty temp
	mIndex_++; //increment mIndex to acount for larger temp
	unsigned int updatedToNotDelete_ = 0; //keep track of size to update at end

	for(HASH_INDEX_T i = 0; i < CAPACITIES[mIndex_-1]; i++){ //iterate through
		if(oldCopy[i] != NULL){ //make sure spot of interest is not NULL
			if(oldCopy[i]->deleted == true){ //if deleted set to true, do memory deletion for hash item
				delete oldCopy[i];
			}else{ //otherwise, copy item into first availablespot in table
				HASH_INDEX_T h = this->probe(oldCopy[i]->item.first);
				table_[h] = oldCopy[i];
				updatedToNotDelete_++;
			}
		}
	...
	toDelete_ = 0;
	toNotDelete_ = updatedToNotDelete_;
}
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if((table_[loc]->deleted != true) && kequal_(key, table_[loc]->item.first)) {
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
